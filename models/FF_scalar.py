...
# new output_size
output_size = 1
image_label_size = max_blobs_train - min_blobs_train + 1
...
correct = tf.cast(tf.argmax(onehot_labels, 1), tf.float64)
prediction = tf.cast(tf.reduce_max(classification, 1), tf.float64)
#print(tf.shape(correct))
#print(tf.shape(prediction)) 
#tf.arg_max(classification, 1)

predcost = tf.square(tf.subtract(correct, prediction))

# all-knower
R = tf.cast(tf.equal(correct, tf.round(prediction)), tf.float32)
...
def evaluate():
    ...
    accuracy = 0
    sumlabels = np.zeros(image_label_size)
...
if __name__ == '__main__':
    ...
    for i in range(start_restore_index, train_iters):
        ...
        if i%100==0:
            #print(tf.shape(sum_rwd))
            #print(tf.shape(sum_pc))
            print("iter=%d : Reward: %f Pc: %f" % (i, sum_rwd/100, 0))#sum_pc/100))
            sum_rwd = 0
            sum_pc = 0
            sys.stdout.flush()
