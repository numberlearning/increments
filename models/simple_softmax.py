...
training_epochs = 2000001
num_test_epochs = 1
checkpoints = [10, 50, 100, 500, 5000, 10000, 20000, 30000, 40000, 50000, 100000, 300000, 500000, 1000000, 1000500, 1001000, 1005000, 1010000, 1050000, 1100000, 1500000, 2000000]
save_dir = "softmax_model_sd0.3_decrsfN_ext_new/"+model_name

restore = False
start_restore_index = 0
load_file = save_dir+"/softmax_model_data_run" + str(num_run)+"_epoch"+str(start_restore_index)+".ckpt"
...
def generate_training():
    ...
    for num in range(1, nMax+1):
        #nImages_train = 1000
        nImages_train = int(15000*(num**(-2))/S)
...
if __name__ == '__main__':
    ...
    def output(x):
        output_layer = tf.nn.softmax(tf.add(tf.matmul(x, weights['output_w']), biases['output_b']))
        return output_layer
    ...
    Y_test = tf.placeholder(tf.float32, shape=[nImages_test*nMax, n_output])
    weights = {
    ...
    ### LOSE FUNCTION ###
    # cross-entropy
    predquality = -tf.reduce_sum(tf.log(y_pred + 1e-5) * y_true, 1)
    tf.cast(predquality, tf.float32)
    predquality_without_nans = tf.where(tf.is_nan(predquality), tf.zeros_like(predquality), predquality)
    cost = tf.reduce_sum(predquality_without_nans)
    ...
    for train_epoch in range(start_restore_index, training_epochs):
        ...
        if train_epoch in checkpoints:
            ...
            for test_epoch in range(num_test_epochs):
                ...
                classifications = sess.run(y_pred_test, feed_dict={X_test: x_test_data, Y_test: y_test_data})
            ...
            w, b = sess.run([weights, biases], feed_dict={X: np.zeros((batch_size, n_hidden)), Y: np.zeros((batch_size, n_output))})
            ...
            print("Model saved in file: %s" % saver.save(sess, save_dir+"/softmax_model_data_run"+str(num_run)+"_epoch"+str(train_epoch)+".ckpt"))

            np.save(save_dir+"/softmax_model_data_run"+str(num_run)+"_epoch"+str(train_epoch)+".npy", model_data)
