...
import numpy as np
import random
from model_settings import img_height, img_width, min_blobs_train, max_blobs_train, min_blobs_test, max_blobs_test # MT
...
def get_size(testing, num_blobs, max_blobs):
    ...
    if testing:
        return 1000 # make 1000 images for each number
    else:
        return 1000 # training: const fN
        #return int(10000*(num_blobs**(-2))/get_S(max_blobs)) # training: decrs fN 
...
def get_k(testing=False):
    ...
    if testing:
        ...
        return 3  # CAA
        #return random.randint(2, 4) # used by both new DAA & old DAA
...
def get_s(testing=False, n=None):
    ...
    if testing:
        ...
        return random.uniform(.5, 1.5) # new CAA
        #return random.uniform(.8*2 * (n ** (-.3154)), 1.2*2 * (n ** (-.3154))) # DAA
        #return random.uniform(.5*2 * (n ** (-.3154)), 1.5*2 * (n ** (-.3154))) # new DAA 
